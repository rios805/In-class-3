        ...
        while (levelRows.Count > 0)
        {
            ...
            for (int col = 0; col < letters.Length; ++col)
            {
               GameObject blockPrefab = null;
                ...
                // Determine which block prefab to use based on the character
                switch (letters[col])
                {
                    case 'r': blockPrefab = rockPrefab; break;
                    case 'b': blockPrefab = brickPrefab; break;
                    case '?': blockPrefab = questionBoxPrefab; break;
                    case 's': blockPrefab = stonePrefab; break;
                    case 'x': blockPrefab = testPrefab; break; 
                }
